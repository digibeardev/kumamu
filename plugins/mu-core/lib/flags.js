...
module.exports = async mu => {
  const { Collection } = require(mu.basePath + "/classes/collection");
  ...
  // Register the flag object with the API system.
  mu.register("flags", new Flags("flags"));

  // Set the schema for adding a new flag.
  mu.flags.schema({
  ...
  // @ts-ignore
  await mu.flags.create(require("../data/flags.json"));
